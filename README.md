<div align="center">

```
╔══════════════════════════════════════════════════════════════╗
║                                                              ║
║                   M A R C O S   M A T E O                    ║
║                                                              ║
║   Backend Engineer  ·  Systems Thinker  ·  API Craftsman     ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
```

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=16&duration=3500&pause=800&color=6E9EF7&center=true&vCenter=true&width=600&lines=Backend+Engineer+%7C+3%2B+years+of+experience;REST+APIs+%E2%80%A2+Automation+%E2%80%A2+Scalable+Systems;Python+%E2%80%A2+ASP.NET+%E2%80%A2+SQL+%E2%80%A2+Cloud;Building+systems+that+scale.)](https://git.io/typing-svg)

</div>

---

## `$ whoami`

Backend engineer with **3+ years** building systems that actually hold up under pressure.  
I focus on what happens behind the interface — clean APIs, reliable data pipelines, and architectures designed to scale.

Currently working across **Python**, **ASP.NET/C#**, and cloud environments, with a growing focus on **data analysis** and **AI-integrated backends**. I'm interested in systems that solve real problems with real constraints.

I believe good backend engineering is invisible — it just works.

---

## `$ cat current_focus.txt`

```yaml
role:       Backend Engineer
focus:
  - REST API design & development
  - Automation pipelines
  - Scalable system architecture
  - Cloud infrastructure (AWS)
  - SQL-first data solutions
  - Data analysis & AI integrations
approach:   Build it clean. Make it scale. Document it properly.
```

---

## `$ ls ./tech_stack/`

**Languages & Frameworks**

![Python](https://img.shields.io/badge/Python-1a1a2e?style=flat-square&logo=python&logoColor=6E9EF7)
![C#](https://img.shields.io/badge/C%23-1a1a2e?style=flat-square&logo=csharp&logoColor=6E9EF7)
![ASP.NET](https://img.shields.io/badge/ASP.NET-1a1a2e?style=flat-square&logo=dotnet&logoColor=6E9EF7)
![Django](https://img.shields.io/badge/Django-1a1a2e?style=flat-square&logo=django&logoColor=6E9EF7)

**Databases**

![SQL Server](https://img.shields.io/badge/SQL_Server-1a1a2e?style=flat-square&logo=microsoftsqlserver&logoColor=6E9EF7)
![MySQL](https://img.shields.io/badge/MySQL-1a1a2e?style=flat-square&logo=mysql&logoColor=6E9EF7)
![MongoDB](https://img.shields.io/badge/MongoDB-1a1a2e?style=flat-square&logo=mongodb&logoColor=6E9EF7)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-1a1a2e?style=flat-square&logo=postgresql&logoColor=6E9EF7)

**Cloud & Infrastructure**

![AWS](https://img.shields.io/badge/AWS_EC2-1a1a2e?style=flat-square&logo=amazonec2&logoColor=6E9EF7)
![Git](https://img.shields.io/badge/Git-1a1a2e?style=flat-square&logo=git&logoColor=6E9EF7)
![Linux](https://img.shields.io/badge/Linux-1a1a2e?style=flat-square&logo=linux&logoColor=6E9EF7)
![Supabase](https://img.shields.io/badge/Supabase-1a1a2e?style=flat-square&logo=supabase&logoColor=6E9EF7)

---

## `$ cat backend_philosophy.md`

```
┌─────────────────────────────────────────────────────────────────┐
│  The backend is where complexity lives.                         │
│  My job is to tame it — with good design, clean contracts,      │
│  and systems that don't surprise you at 2am.                    │
│                                                                 │
│  Data integrity > clever code.                                  │
│  Observability > assumptions.                                   │
│  Simplicity > premature optimization.                           │
└─────────────────────────────────────────────────────────────────┘
```

**What I care about:**
- API contracts that don't break consumers
- SQL queries that don't cause DBA nightmares
- Automation that eliminates manual toil
- Systems designed for the next developer, not just the next sprint

---

## `$ cat roadmap.yml`

```yaml
currently_learning:
  - Azure ecosystem & cloud-native patterns
  - Power Platform (automation & integration)
  - Data Analysis with Python (pandas, visualization)
  - AI-augmented backend architectures
  - Scalable system design principles

next_on_queue:
  - Data pipelines & ETL architecture

long_term_direction:
  - Backend + Data Engineering hybrid profile
  - AI-integrated system design
  - Cloud-native architecture expertise
```

---

<!-- ## `$ gh stats --user=kmbmarcos`


<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=marcosmateo&theme=tokyonight&hide_border=true&background=0d1117&ring=6E9EF7&fire=6E9EF7&currStreakLabel=6E9EF7)](https://git.io/streak-stats)

</div> -->

---

## `$ wakatime --summary`

<div align="center">

[![wakatime](https://wakatime.com/badge/user/f68f6773-2c77-49c5-90d2-d3fb829ee671.svg)](https://wakatime.com/@f68f6773-2c77-49c5-90d2-d3fb829ee671)

</div>

---

## `$ ping contact`

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0d1117?style=for-the-badge&logo=linkedin&logoColor=6E9EF7)](https://www.linkedin.com/in/marcos-mateo-tiburcio-7013991ba/)
[![Email](https://img.shields.io/badge/Email-0d1117?style=for-the-badge&logo=gmail&logoColor=6E9EF7)](mailto:marcosmateot@gmail.com)
[![Instagram](https://img.shields.io/badge/Instagram-0d1117?style=for-the-badge&logo=instagram&logoColor=6E9EF7)](https://www.instagram.com/__jayhock__/)

</div>

---

<div align="center">

```
"Complexity is the enemy of reliability.
 The best system is the one that needs the least explanation."
```

*— Working on it, one commit at a time.*

</div>

---

<div align="center">
<sub>Profile views: <img src="https://komarev.com/ghpvc/?username=marcosmateo&color=6E9EF7&style=flat-square&label=visits" /></sub>
</div>
